...
const Register = () => {
    ...
    // Clear error when user starts typing email again
    useEffect(() => {
        return () => {
            // This will clear the error when component unmounts
            dispatch({ type: 'auth/clearError' });
        };
    }, [dispatch]);
    ...
    const handleSubmit = async (e) => {
        ...
        // Only proceed if not already signing up
        ...
        try {
            ...
            // If there's an error and it's a duplicate email
            ...
            setIsSigningUp(false); // Always re-enable the button
        }
    }

    return (
        <RegisterContainer>
            <BackgroundImage src={register} alt="Background" />
            <FormContainer>
                    ...
                    <div className="w-full">
                        <div className="inputForm">
                            ...
                            <input
                                ...
                                className={`input w-full ${error && error.includes('already exists') ? 'border-red-500' : ''}`}
                                required
                            />
                        </div>
                        {error && error.includes('already exists') && (
                            <p className="text-red-500 text-sm mt-1 ml-2">There is already an account with that email</p>
                        )}
                    </div>
                    <div className="w-full">
                        <div className="flex-column">
